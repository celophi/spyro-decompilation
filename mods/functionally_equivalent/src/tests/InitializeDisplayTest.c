...
static func decompiledFunction = (func) &InitializeDisplay;

/// @brief Address of the original function to invoke.
extern int OG_InitializeDisplay;
static func originalFunction = (func) &OG_InitializeDisplay;
...
static void Tester()
{
    ...
    // Record results.
    printf("InitializeDisplay Test Result: %d/%d\n", hits, total);
    
    LeaveCriticalSection();
}

/// @brief Hook installation entry point.
void InstallInitializeDisplayTest()
{
