...
static func decompiledFunction = (func) &InitializeMemoryCards;

/// @brief Address of the original function to invoke.
extern int OG_InitializeMemoryCards;
static func originalFunction = (func) &OG_InitializeMemoryCards;
...
static void Tester()
{
    ...
    // Record results.
    printf("InitializeMemoryCards Test Result: %d/%d\n", hits, total);
    
    LeaveCriticalSection();
}

/// @brief Hook installation entry point.
void InstallInitializeMemoryCardsTest()
{
